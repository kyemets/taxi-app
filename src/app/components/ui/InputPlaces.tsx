	...
	return (
		...
		{({ getInputProps, suggestions, getSuggestionItemProps, loading }) => (
			<div className={cn('shadow-lg', {
				...
				<div className='py-4 px-5 bg-white rounded-lg flex items-center' style={
					suggestions.length ? {
				...
				>
					...
					{ !isFrom && (
							<div className='absolute right-5 text-sm text-gray-400'>{travelTime ? `${travelTime}min. (${Math.ceil(travelTime / 60)}) h.` : '-min.'}</div>
						)}
				</div>
						...
						suggestions.map((suggestion, index) => (
							<div
								{...getSuggestionItemProps(suggestion, {
									className: cn('cursor-pointer p-3 text-black', {
